...
SYM_INLINE static void init(QwtPlot *plot)
...
SYM_INLINE static void init(QwtPlot *plot,double xMin,double xMax,double yMin,double yMax)
{ return QwtPlotHelper::init(plot,xMin,xMax,yMin,yMax); }

template<typename T_x,typename T_y>
SYM_INLINE static void init(QwtPlot *plot,const T_x xMin,const T_x xMax,const T_y yMin,const T_y yMax)
{ return QwtPlotHelper::init<T_x,T_y>(plot,xMin,xMax,yMin,yMax); }

#endif // QWT_PLOT_HELPER_H
