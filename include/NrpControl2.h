...
/*                          N r p C o n t r o l                               */
/*----------------------------------------------------------------------------*/
/*      (C) Copyright 2003-2015 Rohde & Schwarz  All Rights Reserved.         */
/*----------------------------------------------------------------------------*/
/*                                                                            */
/* Title:       NrpControl2.h                                                 */
...
/*
 * Version Informations (from 'NrpControl2Version.h')
 */
#ifndef NRP_VAL_MAJOR_VERSION
  #define NRP_VAL_MAJOR_VERSION                   3
  #define NRP_VAL_MINOR_VERSION                   71
  #define NRP_VAL_BUILD_VERSION                   0
  #define NRP_VAL_SUB_VERSION                     0
#endif
...
#define NRP_ERROR_SENSOR_IN_USE             ( NRP_ERROR_CONTROL_OFFSET + 34 ) /* 0xC0000022 */
...
#define NRP_ATTR_HIGHSPEED                  ( 0x3FFF0020L )
...
#ifdef UNDER_CE
long _NRP_FUNC NrpScan( void );
#endif

#ifndef UNDER_CE

long _NRP_FUNC NrpGetUsageMap( char *cpMap, unsigned long nMapSize, unsigned long *pulRetSize );
...
//==============================================
//==============================================
//     Special purposes / Internal use only
//==============================================
//==============================================
/**
typedef struct tag_EP
{
...
typedef struct tag_IF
{
  unsigned char  bLength;
  unsigned char  bDescriptorType;
  unsigned char  bInterfaceNum;
...
typedef struct tag_PhysicalConfigurationDesc
{
  unsigned char  bLength;
  unsigned char  bDescriptorType;
  ...
  // data for selecting a device
  // (This ain't part of the physical descriptor;
  //  it is only used for selecting a device by
  //  its vendor/product ID)
  unsigned short usSelectVendorID;
  unsigned short usSelectProductID;
} tPhysicalConfigurationDesc;
*/
//==============================================

// Since Windows may hide the real (physical) configuration
// descriptor in case of a composite devices, we cannot use
// the standard GetDescriptor() calls to read a device's
// real (physical) configuration descriptor. -- This function
// walks along the USB tree searching for a specific device
// and returns its real physical descriptor
//long _NRP_FUNC NrpGetPhysicalConfigurationDescriptor( tPhysicalConfigurationDesc *pDesc );
long _NRP_FUNC NrpGetPhysicalConfigurationDescriptor( void *pDesc );
