...
radio_6449 g_radio_6449[g_channels];
#define g_radio g_radio_6449[index]
...
{
    return radio_6449::set_init_callback(callback);
}
...
int32_t radio::init()
{
    ...
    radio_6449::set_en_expt(true);

    LM(radio_6449::_init_callback(),"error");
    ...
    for (uint32_t i = 0;i < channels();++i) {
        g_radio_6449[i].set_rf_idx(i);
        g_radio_6449[i].connect(serial);
        g_radio_6449[i].open_board();
    }
