...
public:
    ...
    typedef std::vector<CSGPMMFile::Point> FreqSeg;
public:
...
public:
	...
	int32_t Commit();
    int32_t To2DArray(std::vector<FreqSeg> &Segs,std::vector<uint64_t> *Freqs = NULL,std::vector<float> *MaxPtPows = NULL);
	int32_t Clear();
private:
